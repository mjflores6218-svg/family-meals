...
Promise.all([
  fetch('data/recipes.json?v=13.3', { cache: 'no-store' }).then((r) => { if (!r.ok) throw new Error('recipes'); return r.json(); }),
  fetch('data/history.json?v=13.3', { cache: 'no-store' }).then((r) => { if (!r.ok) throw new Error('history'); return r.json(); })
]).then(([r, h]) => {
...
// v13.3: planning summary for ChatGPT / weekly planning
